...
FaceStore = TypedDict("FaceStore", {"static_faces": FaceSet})
...
VideoPoolSet = TypedDict(
    "VideoPoolSet", {"capture": VideoCaptureSet, "writer": VideoWriterSet}
)
CameraPoolSet = TypedDict("CameraPoolSet", {"capture": CameraCaptureSet})
...
AudioEncoder = Literal[
    "flac", "aac", "libmp3lame", "libopus", "libvorbis", "pcm_s16le", "pcm_s32le"
]
...
EncoderSet = TypedDict(
    "EncoderSet", {"audio": List[AudioEncoder], "video": List[VideoEncoder]}
)
VideoPreset = Literal[
    "ultrafast",
    "superfast",
...
ExecutionProvider = Literal[
    "cpu", "coreml", "cuda", "directml", "openvino", "migraphx", "rocm", "tensorrt"
]
...
ValueAndUnit = TypedDict("ValueAndUnit", {"value": int, "unit": str})
ExecutionDeviceFramework = TypedDict(
    "ExecutionDeviceFramework", {"name": str, "version": str}
)
ExecutionDeviceProduct = TypedDict(
    "ExecutionDeviceProduct", {"vendor": str, "name": str}
)
ExecutionDeviceVideoMemory = TypedDict(
    "ExecutionDeviceVideoMemory",
    {"total": Optional[ValueAndUnit], "free": Optional[ValueAndUnit]},
)
ExecutionDeviceTemperature = TypedDict(
    "ExecutionDeviceTemperature",
    {"gpu": Optional[ValueAndUnit], "memory": Optional[ValueAndUnit]},
)
...
DownloadProviderValue = TypedDict(
    "DownloadProviderValue", {"urls": List[str], "path": str}
)
DownloadProviderSet: TypeAlias = Dict[DownloadProvider, DownloadProviderValue]
DownloadScope = Literal["lite", "full"]
Download = TypedDict("Download", {"url": str, "path": str})
...
JobStore = TypedDict("JobStore", {"job_keys": List[str], "step_keys": List[str]})
...
JobStep = TypedDict("JobStep", {"args": Args, "status": JobStepStatus})
Job = TypedDict(
    "Job",
    {
        "version": str,
